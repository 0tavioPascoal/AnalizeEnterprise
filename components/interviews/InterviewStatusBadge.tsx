...
type InterviewStatus = "ready" | "generating" | "failed" | "draft";
...
export function InterviewStatusBadge({ status }: InterviewStatusBadgeProps) {
  const currentStatus = getInterviewStatus(status);
  ...
  return (
    ...
    >
      <Icon
        size={13}
        className={cn(currentStatus === "generating" && "animate-spin")}
      />
...
function getInterviewStatus(status?: string | null): InterviewStatus {
