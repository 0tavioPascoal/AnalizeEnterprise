  ...
  return (
    ...
    >
      <div className="flex h-full flex-col overflow-hidden">
        <Tabs
          value={activeStatus}
          onValueChange={(value) => setActiveStatus(value as PipelineStatus)}
          className="flex flex-1 flex-col overflow-hidden"
        >
          <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <TabsList className="h-12 w-fit rounded-xl bg-zinc-100 p-1 dark:bg-zinc-800">
              <TabsTrigger value="pending" className="gap-2 rounded-lg px-6 text-[10px] font-black uppercase tracking-widest">
                <Clock size={14} />
                Pendentes
              ...
              <TabsTrigger value="approved" className="gap-2 rounded-lg px-6 text-[10px] font-black uppercase tracking-widest data-[state=active]:text-emerald-600">
              ...
              <TabsTrigger value="rejected" className="gap-2 rounded-lg px-6 text-[10px] font-black uppercase tracking-widest data-[state=active]:text-red-600">
                <XCircle size={14} />
                Reprovados
                <span className="rounded-md bg-zinc-200 px-2 py-0.5 text-[10px] dark:bg-zinc-700">
                  {counts.rejected}
          ...
          {statuses.map((status) => (
            <TabsContent key={status} value={status} className="mt-0 flex-1 overflow-hidden">
                ...
                {loading ? (
                  ...
                  <div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
                    ...
                    <p className="max-w-sm text-sm text-zinc-500">
                      Não existem análises em “{statusLabels[status]}” para o filtro atual.
                    </p>
                  </div>
                ) : (
                    ...
                    <table className="w-full border-collapse text-left">
                      <thead className="sticky top-0 z-10 bg-zinc-50/90 backdrop-blur-md dark:bg-zinc-800/90">
                        <tr className="border-b border-zinc-100 dark:border-zinc-800">
                          ...
                          <th className="p-5 text-center text-[10px] font-black uppercase tracking-widest text-zinc-400">Score Match</th>
                          <th className="p-5 text-right text-[10px] font-black uppercase tracking-widest text-zinc-400">Ação</th>
                        </tr>
                      </thead>
                            ...
                            <td className="p-5 text-center">
                              <span
                                className={cn(
                                  ...
                                  status === "approved" && "border-emerald-100 bg-emerald-50 text-emerald-600",
                                  status === "rejected" && "border-red-100 bg-red-50 text-red-600",
                                  status === "pending" && "border-zinc-100 bg-zinc-50 text-zinc-500",
                                )}
                              >
                                {item.score}%
                              </span>
